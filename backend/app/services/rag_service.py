...
from app.services.retrieval_strategies import retrieve, requery_if_needed
...
## 回答（请在引用文档内容时标注来源序号，如 [1]。回答结束后，在最后一行单独输出 [SUFFICIENT] 表示检索内容足以回答此问题，或 [INSUFFICIENT] 表示不足）"""
...
def _build_sources_and_context(items: list, kb_ids: list, db=None):
    """从检索结果构建 sources 和 context 文本"""
    all_contexts = [it["text"] for it in items]
    sources = [{
        "text": it["text"][:150],
        "score": round(max(filter(None, [it.get("final_score"), it.get("dense_score"), it.get("rerank_score"), it.get("score")])), 3) if any(it.get(k) for k in ["final_score","dense_score","rerank_score","score"]) else 0,
    ...
    if sources and kb_ids:
        ...
        try:
            ...
            kb_map = {kb.id: kb.name for kb in _db.query(KnowledgeBase).filter(KnowledgeBase.id.in_(kb_ids_in_sources or kb_ids)).all()} if kb_ids_in_sources else {}
            doc_map = {}
            for s in sources:
...
async def rag_query(
    ...
    if db:
        try:
            from app.services.faq_stats_service import check_stats_cache
    ...
    # ---- 第一轮：直接检索 ----
    items = await retrieve(question, kb_ids, top_k=8)
    ...
    source_info, ctx, all_contexts = _build_sources_and_context(items, kb_ids, db)
    ...
    need_requery = full_stripped.endswith("[INSUFFICIENT]")
    if need_requery:
        # 清理掉 [INSUFFICIENT] 标记，不展示给用户
        full = full_stripped.replace("[INSUFFICIENT]", "").strip()
        logger.info("Answer too short (%d chars), trying corrective requery", len(full))
        new_items = await requery_if_needed(question, kb_ids, items, top_k=5)
        if new_items:
            items = items + new_items
            new_si, new_ctx, new_contexts = _build_sources_and_context(items, kb_ids, db)
            source_info = new_si
    ...
    if db:
        try:
            ...
            from app.services.faq_stats_service import record_query
