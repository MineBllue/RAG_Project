...
RAG_PROMPT = """你是一个企业知识库智能助手。请根据以下知识库内容回答用户问题。要求：基于内容回答，专业准确简洁，使用中文。
...
async def rag_query(
    ...
    if use_rerank and len(items) > 1:
        try:
            ...
            # 只用 rerank 值如果它比原始分高（避免兜底 0.5 覆盖真实高分）
            rm = {r["text"]: r.get("rerank_score", 0) for r in ranked}
            for it in items:
                rr = rm.get(it["text"], 0)
                if rr > 0.6:  # rerank 值 > 0.6 才采纳
                    it["final_score"] = rr
